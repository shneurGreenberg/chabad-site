...
class CloudSync {
  ...
  Future<String?> push({
    ...
    try {
      ...
      await db.collection('site').doc('content').set({
        ...
        'shiurim': snapshot['shiurim'],
        'famous': snapshot['famous'],
        'history': snapshot['history'],
        'tour': snapshot['tour'],
        'emblemUrl': snapshot['emblemUrl'],
        'imageKeys': mediaIds,
      });
  ...
  Future<void> _syncSubscribers(
    ...
    for (final d in existing.docs) {
      if (prune && !keep.contains(d.id)) await d.reference.delete();
    }
  }
